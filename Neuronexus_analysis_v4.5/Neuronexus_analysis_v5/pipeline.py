...
class PipelineConfig:
    ...
    filter_order: int = 4              # IIR用
    filter_fir_numtaps: Optional[int] = None  # FIR用タップ数（None=自動計算）
    ...
    fft_freq_max: float = 300.0        # FFT最大周波数（表示用）

    ica_components: bool = True
    power_analysis: bool = True
    power_freq_min: float = 0.5        # パワースペクトル表示最小周波数
    power_freq_max: float = 100.0      # パワースペクトル表示最大周波数
...
def run_pipeline(config: PipelineConfig) -> Dict[str, Any]:
    ...
    if config.filter_enabled:
        ...
        log(f"  バンドパス: {config.filter_lowcut}-{config.filter_highcut}Hz [{filter_desc}]")
        if config.notch_enabled:
            log(f"  ノッチフィルタ: {config.notch_freq}Hz (Q={config.notch_Q}) [IIR]")
    else:
