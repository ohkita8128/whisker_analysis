...
def plot_processing_overview(lfp_trimmed, lfp_filtered, lfp_cleaned, lfp_times,
    ...
    # モーション
    axes[0].plot(times_plot, motion_resampled[t_mask], 'b-', lw=0.5)
    axes[0].axhline(motion_threshold, color='r', ls='--')
    axes[0].fill_between(times_plot, 0, motion_resampled[t_mask].max(), where=noise_mask[t_mask], alpha=0.3, color='purple')
    axes[0].set_ylabel('Motion')
    axes[0].set_title('Motion Detection')
   
    # Raw LFP
    for idx in range(n_channels):
        axes[1].plot(times_plot, lfp_trimmed[t_mask, idx] - idx*0.3, lw=0.5)
    axes[1].set_ylabel('LFP')
    axes[1].set_title('Raw LFP')
    
    # Filtered LFP
    for idx in range(n_channels):
        axes[2].plot(times_plot, lfp_filtered[t_mask, idx] - idx*0.3, lw=0.5)
    axes[2].set_ylabel('LFP')
    axes[2].set_title('Filtered LFP')
    
    # ICA Cleaned LFP
    for idx in range(n_channels):
        axes[3].plot(times_plot, lfp_cleaned[t_mask, idx] - idx*0.3, lw=0.5)
    axes[3].fill_between(times_plot, -5, 0.5, where=noise_mask[t_mask], alpha=0.3, color='purple')
...
def plot_power_analysis_color(freqs, psd_baseline, psd_stim, psd_post,
...
def plot_power_analysis(freqs, psd_baseline, psd_stim, psd_post,
                        bands, baseline_power, stim_power, post_power,
                        change_stim_list, change_post_list,
                        output_dir, basename, power_freq_min=0.5, power_freq_max=100.0, 
                        n_peaks=3, show=True, save=True):
    """
    パワースペクトル解析プロット（上段のみ帯域背景色付き）
    """
    ...
    # 帯域の色（GUIのBandEditorFrameと同じ順番・同じ色）
    BAND_COLORS = ['#3b82f6', '#22c55e', '#f59e0b', '#a855f7', '#ef4444', '#06b6d4', '#ec4899', '#84cc16']
...
def plot_wavelet_single(cwt_power, cwt_freqs, lfp_times, stim_times,
    ...
    for stim_t in stim_in_range:
        # 太い線 + 黒い縁取り
        ax.axvline(stim_t, color='black', alpha=0.8, lw=3)
        ax.axvline(stim_t, color='yellow', alpha=0.9, lw=2)
        # 上部に三角マーカー
        ax.plot(stim_t, cwt_freqs[-1], 'v', color='yellow', markersize=8, 
                markeredgecolor='black', markeredgewidth=1, clip_on=False)
