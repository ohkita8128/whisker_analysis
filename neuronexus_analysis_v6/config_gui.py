...
class ConfigGUI:
    ...
    def _build_ui(self):
        ...
        self.notebook.add(tab_spike, text=' 🧠 スパイク ')
        self._build_tab_spike(tab_spike)
    ...
    # タブ4: 🆕 スパイク・位相ロック
    # ================================================================
    def _build_tab_spike(self, parent):
        frame = self._create_scrollable_frame(parent)
        row = 0
        
        # スパイク解析
        row = self._add_section_header(frame, "⚡ スパイク解析", row, group_key="spike")
        row = self._add_checkbox(frame, "spike_analysis_enabled", "スパイク解析を実行", row, default=False, group="spike")
        
        # スパイク品質フィルタ（カスタム実装）
        quality_frame = ttk.Frame(frame)
        quality_frame.grid(row=row, column=0, columnspan=2, sticky='w', padx=5, pady=2)
        ttk.Label(quality_frame, text="スパイク品質フィルタ:").pack(side='left')
        quality_var = tk.StringVar(value="sorted_only")
        self.vars["spike_quality_filter"] = {"var": quality_var, "type": "str", "default": "sorted_only"}
        ...
        ttk.Label(quality_frame, text="(sorted_only: ソート済みのみ, mua: マルチユニット)", 
                 font=('', 8), foreground='gray').pack(side='left')
        row += 1
        
        # 位相ロック解析
        row = self._add_section_header(frame, "🔄 位相ロック解析", row, group_key="phase_locking")
        row = self._add_checkbox(frame, "phase_locking_enabled", "位相ロック解析を実行", row, default=False, group="phase_locking")
        row = self._add_number_input(frame, "phase_locking_lfp_channel", "参照LFPチャンネル", row, default=0, is_int=True)
        row = self._add_number_input(frame, "phase_locking_min_spikes", "最小スパイク数", row, default=50, is_int=True)
        row = self._add_checkbox(frame, "phase_locking_condition_analysis", "条件別解析 (baseline/stim/post)", row, default=True, group="phase_locking")
        row = self._add_number_input(frame, "stim_artifact_window", "刺激アーティファクト除外窓 (秒)", row, default=0.005)
        
        # 位相ロック周波数帯域（簡易設定）
        band_frame = ttk.LabelFrame(frame, text="📊 位相ロック周波数帯域")
        band_frame.grid(row=row, column=0, columnspan=2, sticky='ew', padx=5, pady=5)
        row += 1
        
        # Theta（辞書形式で格納）
        theta_frame = ttk.Frame(band_frame)
        theta_frame.pack(fill='x', padx=5, pady=2)
        pl_theta_enabled_var = tk.BooleanVar(value=True)
        self.vars["pl_theta_enabled"] = {"var": pl_theta_enabled_var, "type": "bool", "default": True}
        ttk.Checkbutton(theta_frame, text="Theta", variable=pl_theta_enabled_var).pack(side='left')
        pl_theta_low_var = tk.StringVar(value="4")
        pl_theta_high_var = tk.StringVar(value="12")
        self.vars["pl_theta_low"] = {"var": pl_theta_low_var, "type": "str", "default": "4"}
        self.vars["pl_theta_high"] = {"var": pl_theta_high_var, "type": "str", "default": "12"}
        ttk.Entry(theta_frame, textvariable=pl_theta_low_var, width=5).pack(side='left', padx=2)
        ttk.Label(theta_frame, text="-").pack(side='left')
        ttk.Entry(theta_frame, textvariable=pl_theta_high_var, width=5).pack(side='left', padx=2)
        ttk.Label(theta_frame, text="Hz").pack(side='left')
        
        # Gamma（辞書形式で格納）
        gamma_frame = ttk.Frame(band_frame)
        gamma_frame.pack(fill='x', padx=5, pady=2)
        pl_gamma_enabled_var = tk.BooleanVar(value=True)
        self.vars["pl_gamma_enabled"] = {"var": pl_gamma_enabled_var, "type": "bool", "default": True}
        ttk.Checkbutton(gamma_frame, text="Gamma", variable=pl_gamma_enabled_var).pack(side='left')
        pl_gamma_low_var = tk.StringVar(value="30")
        pl_gamma_high_var = tk.StringVar(value="80")
        self.vars["pl_gamma_low"] = {"var": pl_gamma_low_var, "type": "str", "default": "30"}
        self.vars["pl_gamma_high"] = {"var": pl_gamma_high_var, "type": "str", "default": "80"}
        ttk.Entry(gamma_frame, textvariable=pl_gamma_low_var, width=5).pack(side='left', padx=2)
        ttk.Label(gamma_frame, text="-").pack(side='left')
        ttk.Entry(gamma_frame, textvariable=pl_gamma_high_var, width=5).pack(side='left', padx=2)
        ttk.Label(gamma_frame, text="Hz").pack(side='left')
        
        # 保存設定
        row = self._add_section_header(frame, "💾 位相ロック出力", row)
        row = self._add_checkbox(frame, "save_phase_locking_plots", "位相ロックプロット保存", row, default=True)
        row = self._add_checkbox(frame, "save_phase_locking_csv", "位相ロックCSV保存", row, default=True)
        
        # 注意書き
        note_frame = ttk.LabelFrame(frame, text="📝 注意")
        note_frame.grid(row=row, column=0, columnspan=2, sticky='ew', padx=5, pady=10)
        note_text = """• PLXファイルにスパイクデータが含まれている必要があります
• 'sorted_only' はソーティング済みユニット（unit_id > 0）のみ解析
• 最小スパイク数が不足するユニットはスキップされます
• 位相ロック解析にはスパイク解析が有効である必要があります"""
        ttk.Label(note_frame, text=note_text, font=('', 9), foreground='gray', 
    ...
    def _get_config(self) -> PipelineConfig:
        ...
        # GUI専用の変数（PipelineConfigには渡さない）
        gui_only_keys = {'pl_theta_enabled', 'pl_theta_low', 'pl_theta_high', 
                         'pl_gamma_enabled', 'pl_gamma_low', 'pl_gamma_high'}
        ...
        # 🆕 位相ロック周波数帯域の処理
        phase_locking_bands = {}
        if "pl_theta_enabled" in self.vars and self.vars["pl_theta_enabled"]["var"].get():
            try:
                low = float(self.vars["pl_theta_low"]["var"].get())
                high = float(self.vars["pl_theta_high"]["var"].get())
                phase_locking_bands['theta'] = (low, high)
            except:
                phase_locking_bands['theta'] = (4, 12)
        
        if "pl_gamma_enabled" in self.vars and self.vars["pl_gamma_enabled"]["var"].get():
            try:
                low = float(self.vars["pl_gamma_low"]["var"].get())
                high = float(self.vars["pl_gamma_high"]["var"].get())
                phase_locking_bands['gamma'] = (low, high)
            except:
                phase_locking_bands['gamma'] = (30, 80)
        
        if phase_locking_bands:
            kwargs['phase_locking_bands'] = phase_locking_bands
        
        # 位相ロック有効時はスパイク解析も有効に
        if kwargs.get('phase_locking_enabled', False):
            kwargs['spike_analysis_enabled'] = True
