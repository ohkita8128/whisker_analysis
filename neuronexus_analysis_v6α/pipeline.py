...
class PipelineConfig:
    ...
    phase_locking_condition_analysis: bool = True  # 条件別解析
    stim_artifact_window: float = 0.005        # 刺激アーティファクト除外窓（秒）
        ...
        if len(spike_data['unit_info']) == 0:
            ...
            if config.phase_locking_enabled:
                ...
                for unit_info in spike_data['unit_info']:
                    ...
                    log(f"\n  ユニット: {unit_key} ({unit_info.n_spikes} spikes)")
                    ...
                    if config.stim_artifact_window > 0:
                        ...
                        if n_removed > 0:
                            log(f"    アーティファクト除外: {n_removed} spikes")
                    ...
                    if config.phase_locking_condition_analysis:
                        ...
                        condition_results = analyze_phase_locking_by_condition(
                            ...
                            min_spikes=config.phase_locking_min_spikes // 2,  # 条件別は緩めに
                            verbose=config.verbose
                        )
