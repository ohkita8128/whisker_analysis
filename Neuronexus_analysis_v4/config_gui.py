...
class ConfigGUI:
    ...
    def _build_tab_preprocess(self, parent):
        ...
        # フィルタ設定
        row = self._add_section_header(frame, "🔧 フィルタ設定", row, group_key="filter")
        ...
        row = self._add_checkbox(frame, "notch_enabled", "ノッチフィルタ (電源ノイズ)", row, default=True, group="filter")
        row = self._add_number_input(frame, "notch_freq", "ノッチ周波数 (Hz)", row, default=60.0)
        
        # 高調波ノイズ除去
        row = self._add_section_header(frame, "🔇 高調波ノイズ除去", row)
        row = self._add_checkbox(frame, "harmonic_removal_enabled", "高調波ノイズ除去 (ピエゾ)", row, default=True)
        ...
        row = self._add_number_input(frame, "harmonic_q", "高調波ノッチフィルタQ値", row, default=50.0)
    ...
    def _build_tab_analysis(self, parent):
        ...
        # FFT設定
        row = self._add_section_header(frame, "📉 スペクトル解析", row)
        row = self._add_number_input(frame, "fft_freq_max", "FFT最大周波数 (Hz)", row, default=300.0)
        row = self._add_number_input(frame, "power_freq_max", "パワー表示最大 (Hz)", row, default=100.0)
    ...
    def _run_pipeline(self):
        ...
        try:
            ...
            msg += f"🔧 フィルタ: {config.filter_lowcut}-{config.filter_highcut}Hz\n"
